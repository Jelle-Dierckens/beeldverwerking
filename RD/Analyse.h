...
public:
	...
	void lbp(const cv::Mat & image, cv::Mat & dst, int radius = 1, double threshold_fraction = 0.04);
	cv::Mat readBayes();
	cv::Mat readAvgTruth();
	...
	void compareHistograms(cv::Mat src, cv::Mat & dst, vector<cv::Point2i> &patches, int patchsize, double comp_threshold);
	vector<float> devideInBins(cv::Mat patch, int bincount);
private:
	LineHandler lh;
